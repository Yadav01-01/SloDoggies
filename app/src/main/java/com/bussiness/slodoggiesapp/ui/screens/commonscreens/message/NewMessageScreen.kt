...
fun NewMessageScreen(
    ...
    BackHandler {
        navController.navigate(Routes.MESSAGE_SCREEN){
            launchSingleTop
            popUpTo(Routes.MESSAGE_SCREEN) {
                inclusive = false
            }
        }
    }
    ...
    Box(modifier = Modifier.fillMaxSize().background(Color.White)) {
        Column(modifier = Modifier.fillMaxSize()) {
            HeadingTextWithIcon(
                textHeading = "New Message",
                onBackClick = { navController.navigate(Routes.MESSAGE_SCREEN){
                    launchSingleTop
                    popUpTo(Routes.MESSAGE_SCREEN) {
                        inclusive = false
                    }
